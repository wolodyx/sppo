...
Эти программы следует различать друг от друга.

% Различие между программами просмотра и редакторами
Программа просмотра не имеют той многой функциональности, что составляет редактор.
Поэтому их внутреннее устройство намного проще и более производительнее в работе.
Очевидно, что редакторы можно использовать для просмотра текстового файла.
Можно подстраховаться, открыв файл в режиме "только для чтения".
Но если вам нужно смотреть и искать в файлах с большим размером, то лучше воспользоваться программой просмотра.

% Общие правила для текстовых файлов
Текстовые данные обладают некоторой структурой, упорядывающей содержимое.
Она отличается для разных типов данных.
...
Такой текст называют *плоским* (plan text).
...
Слова разделяются на переменные, названия функций, операторы, ключевые слова, строки.
